...
import { ComponentCard } from "./component-grid";
...
export default function AllComponentsView({
  stats,
}: {
  ...
  useEffect(() => {
    if (typeof window !== "undefined") {
      ...
      if (
        typeParam === "blocks" ||
        categoryParam?.toLowerCase() === "blocks"
      ) {
  ...
  // Staggered choreography values across [0px -> 150px]
  ...
  // Phase 2: Notch container converges inward smoothly into compact floating notch
  ...
  const barBorderBottomRadius = useTransform(
    smoothScrollY,
    [10, 130],
    ["0px", "22px"],
  );
  ...
  // Floating notch glass background & layered shadow (fades in only upon scrolling)
  const notchBgOpacity = useTransform(smoothScrollY, [25, 110], [0, 1]);

  // Back to Home button size adjustment
  const backBtnSize = useTransform(smoothScrollY, [0, 150], ["38px", "34px"]);

  // Symmetrical Left Slot to guarantee exact dead-center search at the top
  ...
  // Search input width & height contraction
  ...
  const allComponentsOnly = useMemo(() => getAllComponents(), []);

  // Extract unique categories
  const categories = useMemo(() => {
    const cats = new Set<string>();
    cats.add("All");
    for (const item of allComponentsOnly) {
      if (item.category) {
  ...
  // Filter components by search & category
  const filteredComponents = useMemo(() => {
    return allComponentsOnly.filter((item) => {
  ...
  }, [allComponentsOnly, searchQuery, selectedCategory]);
  ...
  const scrollToResults = () => {
    ...
    // Only auto-scroll when the navbar is in floating pill mode (scrolled down), not in default top state
  ...
  return (
    <div className="min-h-screen bg-[var(--bg-primary)] text-[var(--text-primary)] flex flex-col">
      {/* Scroll-Morphing Sticky Header with fixed layout footprint */}
      <div
        style={{
      ...
      >
        ...
        >
          ...
          >
            ...
            {/* Center: Search Input in Nav - Truly Dead Center */}
            <motion.div
              style={{
            ...
            <div className="flex shrink-0 items-center justify-end gap-1.5 sm:gap-2">
              {/* Mobile Category Filter Button (visible only on mobile) */}
              <motion.button
                ref={filterButtonRef}
                type="button"
                onClick={() => {
              ...
              {/* Morphing Slot: Seamlessly transforms between Count Badge and Back-to-Top with no layout jump */}
              <motion.div
                style={{
              ...
              >
                ...
                >
                  <span>{allComponentsOnly.length} Components</span>
                </motion.div>

                {/* Back to Top (always full default size in original shape) */}
                <motion.button
                  type="button"
                  onClick={scrollToTop}
            ...
            {/* Mobile Category Filter Modal (Full Notch Width with side breathing room, realistic shadow, and hidden scrollbar) */}
            <AnimatePresence>
              {isFilterOpen && (
                <motion.div
                  ref={filterMenuRef}
      ...
      <main className="flex-1 w-full max-w-7xl mx-auto px-4 sm:px-8 pt-5 sm:pt-8 pb-12 sm:pb-16 lg:pb-20">
        ...
        {/* Component Grid */}
        <div ref={gridRef} id="components-grid" className="scroll-mt-28">
          {filteredComponents.length > 0 ? (
            ...
            <div className="flex flex-col items-center justify-center py-20 text-center rounded-3xl border border-dashed border-[var(--card-border)]">
              ...
              <p className="text-xs sm:text-sm text-[var(--text-muted)] max-w-sm mb-4">
                We couldn&apos;t find any components matching &ldquo;
                {searchQuery}
                &rdquo;.
              </p>
              <button
