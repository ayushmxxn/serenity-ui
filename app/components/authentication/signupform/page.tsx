...
import SignUpFormCode from './components/SignUpFormCode';
          ...
          <div>
            {activeTab === 'Preview' && (
              <div className='black-grid-embed'>
                  <SignUpForm mode={darkMode? 'dark' : 'light'}/>
              </div>
            )}
            {activeTab === 'Code' && (
              <div>
        ...
        <div>
          <div className='relative sm:pl-5'>
                  <pre className='bg-[#18181B] p-3 rounded-md overflow-auto text-sm sm:text-base w-[350px] sm:w-[600px] border border-zinc-700'>
                    <code className='text-zinc-300'>npx @ayushmxxn/serenity-ui@latest add sign-up-form</code>
                  </pre>
                  <button
                    onClick={() => copyToClipboard('npx @ayushmxxn/serenity-ui@latest add sign-up-form', 1)}
                    className='absolute right-0 top-2 p-2 w-10 h-auto rounded bg-[#18181B] border-r border-zinc-700'
