...
// ─── USB-C Cable Component ───────────────────────────────────────────────────

const UsbCable = React.memo(function UsbCable({ color }: { color: string }) {
  const chassisBottom = CHASSIS_TOP_Y - CHASSIS_THICK;
  const deskY = chassisBottom + 0.008;
  const portY = CHASSIS_TOP_Y * 0.5;

  const curve = useMemo(
    () =>
      new THREE.CatmullRomCurve3([
        new THREE.Vector3(0, portY, 0),
        new THREE.Vector3(0.006, portY * 0.4, -0.09),
        new THREE.Vector3(0.016, deskY, -0.22),
        new THREE.Vector3(-0.018, deskY, -0.52),
        new THREE.Vector3(0.024, deskY, -0.88),
        new THREE.Vector3(0.01, deskY, -1.35),
      ]),
    [deskY, portY],
  );

  return (
    <group position={[DISPLAY_X, 0, -CHASSIS_D / 2]}>
      {/* 1. Matte Black Rectangular Plug Body (Plugs flush into chassis, matching reference image) */}
      <RoundedBox
        args={[0.04, 0.02, 0.062]}
        radius={0.0025}
        smoothness={3}
        position={[0, portY, -0.031]}
        castShadow
      >
      ...
      {/* 2. Cylindrical Strain Relief Boot / Collar (matching reference image) */}
      <mesh
        position={[0, portY, -0.075]}
        rotation={[Math.PI / 2, 0, 0]}
        castShadow
      >
        <cylinderGeometry args={[0.0088, 0.0088, 0.026, 24]} />
        <meshStandardMaterial
          color="#161719"
          roughness={0.82}
          metalness={0.08}
      ...
      {/* 3. Original Orange Braided Cable extending across the desk surface as before */}
      <mesh position={[0, 0, -0.082]} castShadow>
        <tubeGeometry args={[curve, 36, 0.014, 12, false]} />
  ...
  return (
    <group>
      ...
      <group rotation={[0, 0, 0]} position={[0, 0, 0]}>
        ...
        {/* USB-C Cable */}
        <UsbCable color={accentColor || KEYBOARD_CONFIG.accents.cableColor} />
...
function ResponsiveCamera() {
  ...
  useEffect(() => {
    ...
    const widthMultiplier = isDesktop ? 1.62 : isTablet ? 1.35 : 1.14;
    const targetWidth = CHASSIS_W * widthMultiplier;
    const targetHeight = isDesktop ? CHASSIS_D * 2.25 : CHASSIS_D * 1.5;
    ...
    const dist = Math.max(baseDistance, requiredDistance);
...
export const Component = MechanicalKeyboardDemo;
export default MechanicalKeyboardDemo;
