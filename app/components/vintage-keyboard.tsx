...
export const VintageKeyboard = ({
  ...
  const activateKey = useCallback(
    (id: string) => {
      clearIndicatorTimeouts();
      setIndicatorVisible(true);
      if (!activeKeyIdsRef.current.includes(id)) {
        activeKeyIdsRef.current.push(id);
      }
      setIndicatorParts(getActiveKeyParts(activeKeyIdsRef.current));
    },
    [clearIndicatorTimeouts],
  );
  ...
  const deactivateKey = useCallback(
    (id: string) => {
      if (!activeKeyIdsRef.current.includes(id)) return;
      activeKeyIdsRef.current = activeKeyIdsRef.current.filter((k) => k !== id);
      if (activeKeyIdsRef.current.length > 0) {
        setIndicatorParts(getActiveKeyParts(activeKeyIdsRef.current));
      } else {
        clearIndicatorTimeouts();
  ...
  return (
    <div
      className={`${
        embedded
          ...
          : "kb-viewport flex w-full items-center justify-center overflow-x-hidden bg-[#FAFAFA]"
      } ${className}`}
      style={{
    ...
    >
      ...
      <div
        ...
        style={{
          width: "100%",
          maxWidth: embedded ? "100%" : container.maxWidth,
        }}
      >
        <div
